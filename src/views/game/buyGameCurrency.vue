<template>
    <div class="buyGameCurrency-page page-layout">
      <van-nav-bar
        :title="$t('buyGameCurrency.t1')"
        left-arrow
      ...
      <div class="wrap">
          <div class="section">
            <div class="title">购买游戏币</div>
            <ul>
              <li v-for="(item) in list" :key="item.id"
                  :class="{'act': item.id === wayId }"
                  @click="changeWay(item)"
              >
                <p >{{ (item.game_number*1).toFixed(0) }}游戏币</p>
                <p >需{{ (item.ctt_number*1).toFixed(0) }}CTT</p>
              </li >
            </ul>
